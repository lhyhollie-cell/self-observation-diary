 ...
 import { useEffect, useState, useRef, type ReactElement } from "react";
  ...
  // 阶段报告字段映射
  const REPORT_LABELS: { [key: string]: string } = {
    "整体状态概览": "整体状态概览",
    "关注主题": "本阶段最突出的关注主题",
    "情绪与焦虑模式": "情绪与焦虑模式",
    "消耗与恢复模式": "消耗与恢复模式",
  ...
  const REPORT_ICONS: { [key: string]: React.ComponentType<{ className?: string }> } = {
    "整体状态概览": Activity,
    "关注主题": Target,
    "情绪与焦虑模式": Heart,
    "消耗与恢复模式": Battery,
  ...
  export default function Home() {
    ...
    const [reportData, setReportData] = useState<{ [key: string]: any } | null>(null);
      ...
      <div className="min-h-screen">
        <main className="max-w-2xl mx-auto px-6 py-12">
          ...
          <div className="flex flex-col gap-3">
            <Link
              href="/record"
              className="w-full text-center px-6 py-3.5 bg-[var(--color-primary)] text-white rounded-xl text-sm font-medium
  hover:bg-[var(--color-primary-hover)] transition-colors"
            ...
            <Link
              href="/reports"
              className="w-full block text-center px-6 py-3 bg-[var(--color-secondary)] text-white rounded-xl text-sm font-medium
  hover:bg-[var(--color-secondary-hover)] transition-colors"
            >
              查看阶段报告
            </Link>
          </div>
